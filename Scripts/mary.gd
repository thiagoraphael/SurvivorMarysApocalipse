...
@export var max_vision_distance = 550
@export var angle_between_rays = 30

@export_group("Attack")
@export_range(0.1, 1) var attack_speed: float = 1
@export_range(1, 10) var attack_damage: float = 10
...
func take_damage(damage: int):
	var reduced_damage = damage - 4
	if reduced_damage < 0:
		reduced_damage = 0  # Garantir que o dano não seja negativo
	health_system.take_damage(reduced_damage)
